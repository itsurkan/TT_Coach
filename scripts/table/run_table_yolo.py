"""
TableDetector YOLO — run YOLOv11-nano-pose on a video and export to _table_yolo.json.
Detects 6 table keypoints (4 corners + 2 net) on a single frame (static camera).

Usage:
    python scripts/run_table_yolo.py                          # all videos
    python scripts/run_table_yolo.py IMG_6332                  # single video
    python scripts/run_table_yolo.py IMG_6332 --samples 5      # try 5 frames
"""
...
MODEL_PATH = PROJECT_ROOT / "models/table_keypoints/best.pt"
...
def process_video(model, video_dir, num_samples):
    """Process one video folder and write _table_yolo.json."""
    ...
    out_path = video_dir / f"{folder_name}_table_yolo.json"
