# Bundled ONNX models — iOS RTMPose backend
...
Matching them byte-for-byte is what lets the iOS exports reuse the same fixtures,
rep-count gates, and baselines as the desktop RTMPose pipeline.

They are **git-ignored** (YOLOX-m is 97 MB, over GitHub's comfort threshold). Fetch
them with `./fetch_models.sh` before building. The build bundles them as resources of
the `TTCoach` target.
...
`rtmlib` downloads and unzips these into `~/.cache/rtmlib/hub/checkpoints/`. `fetch_models.sh`
copies them from there if present, otherwise downloads + unzips from the URLs above, then
verifies the SHA-256s.
