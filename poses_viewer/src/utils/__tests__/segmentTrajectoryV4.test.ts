...
import { segmentTrajectoryV4, predictTrajectoryV4 } from '../trajectoryPipelineV4'
import type { TrajectorySegment } from '../trajectoryPipeline'
...
describe('V4 trajectory segmentation — IMG_6330', () => {
  ...
  it('frames 98-105: arc1 98-102 table_contact, arc2 103-105', () => {
    ...
    const segments = segmentTrajectoryV4(subset, intervalMs)
  ...
  it('frames 114-124: arc1 114-116 table_contact, arc2 117-119 racket_contact, arc3 121-123 table_contact', () => {
    ...
    expect(segments[2].endFrameIndex).toBe(123)
    expect(contactLabel(segments[2])).toBe('table_contact')
  })
})

describe('V4 predictive trajectory — IMG_6330 frames 195-204', () => {
  const { frames, intervalMs } = loadBallData('IMG_6330')
  const videoSize = { width: 1072, height: 1920 }

  // Show ball data for the range
  it('print ball detections 190-210', () => {
    for (let fi = 190; fi <= 210; fi++) {
      const f = frames.find(fr => fr.frameIndex === fi)
      if (!f) { console.log(`f${fi}: NO FRAME`); continue }
      if (!f.ball) { console.log(`f${fi}: NO BALL`); continue }
      console.log(
        `f${fi}: x=${f.ball.x.toFixed(4)} y=${f.ball.y.toFixed(4)}` +
        ` r=${f.ball.radiusPx?.toFixed(1)} conf=${f.ball.confidence.toFixed(2)}`
      )
    }
  })

  // Test predictive trajectory at each frame 195-204
  it('trajectory evolution frame by frame', () => {
    const allFrames = sliceFrames(frames, 0, 210)
    for (let cf = 195; cf <= 204; cf++) {
      const traj = predictTrajectoryV4(allFrames, cf, intervalMs, 10, videoSize)
      if (!traj) {
        console.log(`frame ${cf}: null (no trajectory)`)
        continue
      }
      const arcFrames = traj.pastPositions
        .filter(p => p.source === 'DETECTED')
        .map(p => p.frameIndex)
      console.log(
        `frame ${cf}: arc [${arcFrames.join(',')}]` +
        ` start=${traj.segmentStartFrame}` +
        ` dets=${traj.detectionCount}` +
        ` pred=${traj.predictedPositions.length}` +
        ` cy=${traj.fit.cy.toFixed(6)}`
      )
    }
  })
})
