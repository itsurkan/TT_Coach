 ...
 *   (All shared-side geometry corrects x by `ViewGeometry.xScale`; never feed pixel coords or
 *   square-normalized coords.)
 * - `score` is per-keypoint confidence in [0, 1]. Shared angle functions gate on `score >= 0.3`
 *   and return null below it — do not inflate scores.
 * - Return an empty list when no person is detected (maps to an empty `PoseFrame2D`, which the
 *   shared detector tolerates).
 */
fun interface PoseBackend {
    /** Synchronous on purpose: called on the camera frame queue, which already applies
     * backpressure by dropping late frames. */
