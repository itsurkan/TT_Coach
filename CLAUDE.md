...
**Plan isolation (assume parallel sessions — preserve all changes):** ALWAYS assume other Claude sessions are working the same repo concurrently, each in its own worktree. Never touch the shared working branch's working tree directly, and never do anything that could discard another session's in-flight edits (no `git checkout -- .`, no `git reset --hard`, no `git stash drop`, no force-anything, no `git add -A`). Work in your own **git worktree** off the current working branch (e.g. `2d-experiments`) — prefer worktrees (`superpowers:using-git-worktrees`) so unrelated in-progress edits stay untouched; existing plan worktrees live under `.claude/worktrees/`. Do the whole plan there, keep the tree clean, commit each logical change.

**Integrate via cherry-pick, not merge:** when the plan is complete and green, bring it back to the working branch by **cherry-picking your specific commits** onto it — do not merge the whole branch (a merge can drag in unrelated state or clobber concurrent work). Cherry-pick only the commits you own, resolve conflicts in favor of preserving both sides, and leave the working branch buildable. Do not start a new plan's work in a worktree that still holds another plan's unmerged changes.
