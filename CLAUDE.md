...
**Plan isolation (parallel work in progress):** there are usually several changes in flight, so execute each plan in its own **git worktree or dedicated branch**, never directly on the shared working branch. Branch off the current working branch (e.g. `2d-experiments`), do the whole plan there, keep the working tree clean, and only **merge back to the working branch when the plan is complete and green**. Prefer worktrees (`superpowers:using-git-worktrees`) so unrelated in-progress edits stay untouched; existing plan worktrees live under `.claude/worktrees/`. Do not start a new plan's work in a worktree that still holds another plan's unmerged changes.
